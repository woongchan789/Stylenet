...
class EncoderCNN(nn.Module):
    def __init__(self, model_name, emb_dim):
        ...
        # REVISE: decoder input에 맞게 encoder fc layer 수정
        # REVISE: vit는 in_features = num_classes이어서 따로 작성
        if model_name == 'vit_base_resnet50_224_in21k':
            self.A = nn.Linear(150528, emb_dim)
        else:
            self.A = nn.Linear(model.fc.in_features, emb_dim)
...
class FactoredLSTM(nn.Module):
    ...
    def forward_step(self, embedded, h_0, c_0, mode):
        ...
        if mode == "factual":
            ...
            o = self.S_ho(o)
            c = self.S_hc(c)
        # REVISE: 왜 romantic LSTM은 주석처리 되있는지 의문
        # REVISE: 그래서 실험은 romantic mode를 열고 돌림
        elif mode == "romantic":
            i = self.S_ri(i)
            f = self.S_rf(f)
            o = self.S_ro(o)
